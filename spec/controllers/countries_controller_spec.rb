...
RSpec.describe CountriesController, type: :controller do
  ...
  #FactoryGirl.create(:symbol) factory to create
  #assigns(:symbol) = instance variable in controller @symbol
  ...
  describe "GET #index" do
    ...
    it 'has countries in the instance variable' do
      ...
      expect(assigns(:countries).first.class).to eq(Country)
    end
  end
  ...
  describe 'PUT #update' do
    ...
    it "sets country instance variable" do
      put :update, id: country.id, country: { name: 'Sweeden' }
      expect(assigns(:country)).to eq(country)
      expect(country.reload.name).to eq('Sweeden')
    end

    it "sets flash on success" do
      put :update, id: country.id, country: { name: 'Sweeden' }
      expect(flash[:success]).to eq('Country Updated!')
    end

    it "redirects to country_path on success" do
      put :update, id: country.id, country: { name: 'Sweeden' }
      expect(response).to redirect_to(country_path(country.id))
    end

    it "renders edit on fail" do
      c = FactoryGirl.create(:country, name: 'Sweeden')
      country
  ...
  describe 'DELETE #destroy' do
    it "destroys instance variable" do
      put :destroy, id: country.id, country: { name: 'Sweeden'}
      expect(assigns(:country)).to eq(country)
    end

    it "sets flash on success" do
      put :destroy, id: country.id, country: {}
      expect(flash[:success]).to eq('Country Deleted!')
    end

    it "redirects to country_path on success" do
      put :destroy, id: country.id
